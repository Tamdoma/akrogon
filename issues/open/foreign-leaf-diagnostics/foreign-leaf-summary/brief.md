...
9. Test: a foreign `merged` leaf under `issues/closed/` with a real fixture worktree, branch and recorded tab is reported and left intact by a cleanup-capable `--all` run: state bytes, worktree, branch and tab all remain.
10. Test: two registered repos each holding foreign leaves, run `--all` from outside any registered checkout twice: each run prints exactly one summary per repo and exits nonzero both times, proving per-repo dedup within an invocation and no suppression across invocations.

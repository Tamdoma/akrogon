...
Excluded: `src/phase.ts`, the debate gate, `learnings/` lesson locations, `chart-issues`, `seed-issue`, `init-issues`, `broadcast-issue` skills (not dispatched by `next`), any file move or existence check.

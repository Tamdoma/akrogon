...
`AKROGON_BASE=ab36dd0e424b5b5041dd251c189639498c72b4a8 git diff --check` is the available targeted diff check. No changed-test runner executes skill prose. The actual changed-flow check is the worker's V1–V8 execution against temporary repositories and substituted gh, recorded with commands in `verification/seed-issue.md`. B owns the full suite and other configured checks.
...
Changed files and reasons: Replaced only worktree `skills/seed-issue/SKILL.md` with standalone GitHub intake matching D1–D4. Leaf verification artifacts and this report are evidence outside the worktree implementation scope.
Tests run: Baseline exposed C2/C4 failure. V1–V8 passed through agent-followed real temporary fixtures and substituted gh: seven successful calls, one controlled failure, 19 invalid routes with zero calls. Exact argv/stdin and shell-literal assertions passed. Targeted `AKROGON_BASE=ab36dd0e424b5b5041dd251c189639498c72b4a8 git diff --check` passed. See `verification/seed-issue.md` and its raw evidence files. B owns the full suite.
Known limitations: Agent-followed prose verification is not automated cross-harness coverage. Live GitHub/authentication were substituted. Unreadable routing used a directory, not permission denial. Framework writer and other skills are outside scope. Temporary fixtures/helpers were removed.
Unverified criteria: No C1–C5 criterion remains unverified within the prescribed semantic/substituted method. Missing-root handling is present but was not separately exercised; live GitHub and execution on other harnesses remain unverified limitations, not claimed coverage.

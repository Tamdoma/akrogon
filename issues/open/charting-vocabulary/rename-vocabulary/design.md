...
Exclusions: the framework and pi-extensions migrations in other-repos.md are owned by the migrate-charts leaves in those repos. The docs picture in docs-picture.md is owned by guide-chart-picture; this leaf only rewords the four existing lines and the README row.
...
## Leaf architecture
Owned surfaces: `skills/chart-issues/SKILL.md`, `skills/chart-issues/assets/*.md`, `src/status.ts` chart rows and header, `tests/status.test.ts` chart fixtures, every `issues/chart/*/` and `issues/closed/*/chart/` folder in this repo, the four docs lines named in the brief, the chart-issues row of `README.md`.

Literal interfaces:
- Chart folder: `issues/chart/<chart-slug>/{CHART.md,INTAKE.md,forks/<fork-slug>.md,slots/<pass>.md}`.
- CHART.md sections in order: `## Destination`, `## Forks taken`, `## Forks open`, `## Fog`, `## Off route`, then the optional trailing line `Handed off <YYYY-MM-DD>`.
- Fork file sections: `# <fork title>`, `## Question` holding one or more Q blocks, `### Carries`, `## Findings`, `## Taken`. A fork is taken when `## Taken` has content; partial answers stay under `## Findings`.
- `src/status.ts` `chartRow`: reads `forks/*.md`, counts files matching `/^## Taken\s*\n\s*\S/m` as taken, counts bullets under `Fog` with the existing `section` helper, header `['CHART', 'TAKEN', 'FOG', 'STAGE', 'AGE']`. Stage rule unchanged: `Handed off` line, else files plus fog greater than zero is `charting`, else `empty`.
- questions.md round shape is unchanged except the words: opening paragraph, Q blocks with exhaustive options and one recommended, pitfalls, reply key, challenge check. Slot exchange file examples become `fork-name-A.md`, `fork-name-B.md`, `fork-name-merged.md`, `fork-name-rebuttal-B.md`, `fork-name-final-check-B.md`.

Exclusions: `src/phase.ts` chart move stays as is. `skills/implement-issue/` and `skills/plan-issue/` are not touched. Leaf designs keep "Binding decisions, verbatim". No dual-format reading in status.ts and no migrate subcommand.

Dependencies: none. The two live charts in `issues/chart/` are migrated on the leaf branch; `charting-vocabulary/CHART.md` already carries its `Handed off` line before the branch is cut.

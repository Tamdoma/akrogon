...
## Re-check after check.fix round 2 (rebase repair)

Prior reviewed head: 65b67f46d854ea73569fe73d9af149d02b5e7c40
Rebased head: 398761c on origin/main 7af5184 (merge-base confirmed 7af5184).

Inspected `git range-diff ab36dd0..65b67f4 7af5184..398761c` over src, tests and plugin: the only content change is the import union in `tests/phase.test.ts`; the other commits replayed identically. Working tree clean, no rebase in progress.

Verdict: nits (unchanged).

- Conflict resolved as recorded: both sides of the import block kept.
- Upstream drift integrated without code change: `prompted` reset in `commitMove`, `requireClean` guards, non-force worktree removal all sit beside the leaf's awaited `completeOwner` and pass the leaf's next and phase tests.
- No defect introduced. N1 to N3 stand.

Verification rerun by me at 398761c: `bun test` 46 pass, 0 fail, 487 assertions; `bun run typecheck` clean; `bun run format` changed nothing.

## Merge attempt 2: pushed

Fetched origin main: unchanged at 7af5184, HEAD 398761c still zero behind, AKROGON_BASE 7af5184. Checks from the round-2 re-check at 398761c reused: `bun test` 46 pass, `bun run typecheck` clean, `bun run format` no change. No reusable lesson from N1 to N3.

`git push origin HEAD:main`: fast-forward 7af5184..398761c, exit 0.
`akrogon phase pull-close merged --slot A`: `moved merged`, `issue complete github`. Neither leaf carried `sources`, so no GitHub close ran. One broadcast-issue writer delegated.
>>>>>>> 2a0227e (add issues)

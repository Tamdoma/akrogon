...
Owned: deletion of `docs/guide.html` and a link check across `docs/*.html`. The check is a script run once and discarded, or a shell one-liner recorded in the completion evidence, that extracts every `href` not starting with `http` or `#` from each page and asserts the file exists in `docs/`. Exclusions: no content edits to any page; a broken link found here is a defect in the owning page leaf and is fixed by editing only that link.

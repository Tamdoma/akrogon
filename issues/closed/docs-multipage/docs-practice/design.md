...
Owned: `docs/in-practice.html`, `docs/limits.html`, `docs/problems.html`, `docs/learn.html`, `docs/cheat.html`. Same shell rule as docs-concepts. `in-practice.html` holds the `day` section followed by the `cases` section from guide.html under a single page heading "In practice"; the seed-to-merge worked example is not part of this leaf and arrives as later intake. The cheat section keeps its `ink-band` class. Exclusions: no new prose, no CSS edits, no changes to other pages or guide.html.

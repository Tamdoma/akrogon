...
Owned: `docs/style.css` (the full CSS block moved out of `docs/guide.html` lines 10 to 203, unchanged except for removing the `<style>` wrapper), `docs/index.html`, `docs/idea.html`. Each page is `<!doctype html>`, head with charset, viewport, title, the Google Fonts link from guide.html and `<link rel="stylesheet" href="style.css">`, then the nav header, a `<main>` holding one section, and the footer from guide.html. The idea page holds the section with id `idea` copied from guide.html. The index page holds the hero section and a short list linking every page with one line each. `guide.html` stays untouched in this leaf; nav links to pages that do not yet exist are written anyway and resolve after the page leaves land. Exclusions: no other section moves, no CSS changes.

...
Owned: `docs/install.html`, `docs/setup.html`, `docs/create.html`, `docs/next.html`, `docs/merge.html`. Same rule as docs-concepts: copy the shell from `docs/idea.html`, hold the section of the same id from guide.html, rewrite same-page anchors to page files. The merge section keeps its `ink-band` class. Exclusions: no CSS edits, no changes to other pages or guide.html.

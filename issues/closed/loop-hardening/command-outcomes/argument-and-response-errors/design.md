# Design: argument-and-response-errors
...
Excluded decisions (owned elsewhere or not applicable here): pull-origin (command-reference); tree-preflight parked hint; dispatch-error-report
...
Owned surfaces: the folder resolution in `nextCommand` (`src/next.ts`, `statSync(...).isDirectory()` check before `requireRepo`), `herdr()` in `src/shell.ts` (guarded parse throwing a `CommandError`-shaped error with argv, stdout and cause), the `finally` in `commitMove` (`src/phase.ts`) wrapping the log failure with the committed phase, tests. Unowned-hook silence stays as is.

Exclusions: README changes belong to `command-reference`; parked hint to `tree-preflight`.

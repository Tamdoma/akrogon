...
```bash
echo '{"summary":"...","whats_new":["...","...","..."]}' | bun ~/.claude/skills/broadcast-issue/scripts/discord-send.js --dry-run
```

3. Send the message only after the preview is correct:

```bash
echo '{"summary":"...","whats_new":["...","...","..."]}' | bun ~/.claude/skills/broadcast-issue/scripts/discord-send.js
```
...
Secret values are loaded from `~/.claude/skills/broadcast-issue/.env`; existing process environment values win over file values.

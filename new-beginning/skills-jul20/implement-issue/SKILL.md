...
4. Read the active `## Review Addendum` first when it exists. Treat its open checklist items and sub-checklists as mandatory repair inputs for the current implementation pass.
5. Read the latest relevant entries in `## Addendum Log` before planning edits or execution. Treat recent addendums as amendments to the implementation contract unless the plan explicitly supersedes them.
6. If the plan is too vague to execute safely, stop and tell the user the issue needs another `consult-issue` implementation consult pass first, which may mean a rebuttal refresh or another synthesis pass depending on what is stale.
...
Read `advisory-quality-standards.md` in this skill's folder and apply it before the QA reviewer pass. This SKILL.md and that sidecar are mirrored across the Claude and Codex skill homes; keep both SKILL.md copies role-equivalent and all four sidecar copies content-identical.
...
- prefer direct wording such as `Next step: Run check-issue in Claude to review the completed implementation.`

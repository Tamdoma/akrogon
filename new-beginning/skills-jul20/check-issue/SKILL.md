...
- If review is clean enough for merge, record the merge-ready verdict and keep the issue in `C-ready` or `C-fix`; a normal review run never advances to `D-merge` on its own.
- Advance `C-ready` or `C-fix` to `D-merge` only on a separate explicitly approved operator invocation. That approval authorizes making reviewed worktree deliverables durable by running `bun issues/.scripts/auto-commit-if-dirty.ts <worktree-name> --message "feat(<slug>): finalize changes"` and, if that command exits successfully, the lifecycle-state advance to `D-merge` plus any checkpointing required by that state write; it never authorizes running `merge-issue`, merging branches, rebasing, pushing, cleanup commits, worktree or branch deletion, broadcast handoff, or any other merge workflow step. Surface the finalize command report verbatim, including skipped issue-folder paths. If the finalize command exits non-zero, surface its output and do not advance to `D-merge`. After the state reaches `D-merge`, stop and tell the operator to run `merge-issue` manually.
...
- At the terminal `C-ready -> D-merge` or `C-fix -> D-merge` approval transition only, run `bun issues/.scripts/auto-commit-if-dirty.ts <worktree-name> --message "feat(<slug>): finalize changes"`, surface its report verbatim, move the lifecycle state to `D-merge` only when it exits successfully, and stop; keep all `C-ready -> C-fix` and other intermediate review state as active workflow state.
- If the same slug exists under multiple prefixes anywhere in that combined state, stop and reconcile to one canonical root before applying `C-ready -> C-fix` or an explicitly approved `C-ready/C-fix -> D-merge`.
...
- Before reading or writing a review artifact, resolve the active worktree root and translate matching `issues/...` references to that worktree.
- Write `## Review Addendum` and `## Addendum Log` updates to the implementation `plan.md` under the active worktree when one exists.
- Do not read the implementation surface from, or write review results into, the primary `main` checkout while an active issue worktree exists.
- Before writing, verify the resolved absolute output path is under the active worktree root; if it resolves under `main`, stop and switch to the worktree path.
...
4. Identify the actual implementation surface:
   - diff if available
...
6. Reflect the review result in the issue root state:
   ...
   - when the composed result is merge-ready and the current phase is already `C-ready`, record `merge-ready, awaiting operator approval`, and hold without calling the transition command
   - a normal review never advances to `D-merge`; `C-ready -> D-merge` or `C-fix -> D-merge` requires a separate explicitly approved operator invocation that first runs `bun issues/.scripts/auto-commit-if-dirty.ts <worktree-name> --message "feat(<slug>): finalize changes"`, surfaces its report verbatim, moves lifecycle state to `D-merge` only if the command exits successfully, and then stops
7. Persist the review result in the active `plan.md` before finishing:
   - maintain exactly one active `## Review Addendum` container in the implementation `plan.md`; inside it create or refresh only your own `### Review - Claude` subsection and never edit the peer's subsection
   ...
   - record the checking agent and addendum author explicitly as `Claude`
...
Read `advisory-quality-standards.md` in this skill's folder and apply it after the blocking review. This SKILL.md and that sidecar are mirrored across the Claude and Codex skill homes; keep both SKILL.md copies role-equivalent and all four sidecar copies content-identical.
...
- Checked by agent: Claude
- Checker LLM: [verified model/session label]
- Addendum authored by: Claude
- Issue state: C-ready | C-fix | D-merge
- Merge readiness: merge-ready, awaiting operator approval | not yet merge-ready
- Summary: [1-2 lines]
...
- Last review date: YYYY-MM-DD
- Checked by agent: Codex
- Checker LLM: [verified model/session label]
- Addendum authored by: Codex
- Issue state: C-ready | C-fix | D-merge
- Merge readiness: merge-ready, awaiting operator approval | not yet merge-ready
- Summary: [1-2 lines]
...
- as the single exception, when the verdict is merge-ready and open advisory items exist, use `Next step: Approve D-merge to finish the issue, or rerun implement-issue first to fix the advisory quality items.`; do not name individual `AQ-` IDs there, and do not apply this exception to blocked verdicts
- use one short, plain sentence
- after a `D-merge` approval, prefer direct wording such as `Next step: Run merge-issue in Claude manually to merge the approved issue.`

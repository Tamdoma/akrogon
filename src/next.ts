...
import { allLeaves, findLeaf, readState, saveState, withLock, withRepoLock, withLeafLocks, dependenciesReady, type Leaf, type State } from './state';
...
function launch(global: GlobalConfig, slot: Slot): { kind: string; args: string[] } {
  ...
  const line: string = global.harnesses[config.harness].replaceAll('{model}', quote(config.model)).replaceAll('{effort}', quote(config.effort));
...
async function ensureWorktree(repo: Repo, leaf: Leaf): Promise<State> {
  ...
  if (leaf.state.worktree !== undefined && leaf.state.worktree !== path) throw new Error(`Worktree path mismatch: ${leaf.state.worktree}`);
  if (existsSync(path)) {
    if (realpathSync(await command(['git', 'rev-parse', '--show-toplevel'], path)) !== realpathSync(path)) throw new Error(`Not a worktree root: ${path}`);
    const expectedCommon: string = resolve(repo.root, await command(['git', 'rev-parse', '--git-common-dir'], repo.root));
    const actualCommon: string = resolve(path, await command(['git', 'rev-parse', '--git-common-dir'], path));
    if (realpathSync(expectedCommon) !== realpathSync(actualCommon) || await command(['git', 'branch', '--show-current'], path) !== leaf.state.slug) throw new Error(`Unrelated worktree: ${path}`);
  } else {
    const branch: Result = await run(['git', 'show-ref', '--verify', '--quiet', `refs/heads/${leaf.state.slug}`], repo.root);
    if (branch.code !== 0 && branch.code !== 1) throw new CommandError(['git', 'show-ref', leaf.state.slug], repo.root, branch);
    await command(branch.code === 0 ? ['git', 'worktree', 'add', path, leaf.state.slug] : ['git', 'worktree', 'add', '-b', leaf.state.slug, path, target(repo)], repo.root);
  }
...
async function activeCount(global: GlobalConfig): Promise<number> {
  ...
  for (const [name, path] of Object.entries(global.repos)) {
    ...
    for (const leaf of allLeaves(repo)) {
      if (live.some(pane => pane.tab_id === leaf.state.tab || (pane.cwd !== null && leaf.state.worktree !== undefined && within(pane.cwd, leaf.state.worktree)))) active.add(`${name}/${leaf.state.slug}`);
    }
  }
  return active.size;
}
async function allocate(global: GlobalConfig, repo: Repo, leaf: Leaf): Promise<State | null> {
  ...
  const expectedWorktree: string = resolve(repo.root, repo.config.worktree_root, leaf.state.slug);
  const matches: Tab[] = liveTabs.filter(tab => tab.tab_id === leaf.state.tab || (tab.label === leaf.state.slug && live.some(pane => pane.tab_id === tab.tab_id && pane.cwd !== null && within(pane.cwd, expectedWorktree))));
  ...
  const tab: Tab = matches.length === 1 ? matches[0] : (await herdr(['tab', 'create', '--cwd', worktree, '--label', state.slug, '--env', `AKROGON_BASE=${await base(repo, worktree)}`, '--no-focus'], z.object({ tab: tabSchema, root_pane: paneSchema }))).tab;
...
function retryable(result: Result): boolean {
  const response = z.object({ error: z.object({ code: z.string(), message: z.string() }) }).parse(JSON.parse(result.stderr));
  return ['agent_prompt_stalled', 'agent_blocked', 'agent_not_ready', 'timeout', 'wait_timeout', 'agent_wait_timeout'].includes(response.error.code);
}
async function dispatchSlot(global: GlobalConfig, repo: Repo, leaf: Leaf, slot: Slot): Promise<void> {
  while (true) {
    ...
    const peer: Slot = slot === 'A' ? 'B' : 'A';
    const seat: Slot = state.attempts[slot] >= 2 ? peer : slot;
    ...
    const args: string[] = ['herdr', 'agent', 'prompt', ready.pane_id, prompt, '--wait', '--until', 'working', '--timeout', '5000'];
    const result: Result = await run(args);
    if (result.code === 0) return;
    if (!retryable(result)) throw new CommandError(args, repo.root, result);
    console.warn(JSON.stringify({ warning: 'prompt failed', slug: state.slug, slot, attempt: attempt.attempts[slot], ...result }));
  }
}
async function dispatchLeaf(global: GlobalConfig, repo: Repo, slug: string, explicit: boolean): Promise<boolean> {
  return withRepoLock(repo, async () => {
    ...
    return withLeafLocks(leaf, async () => {
      ...
      for (const slot of requiredSlots(state.phase, state.fix_rounds)) await dispatchSlot(global, repo, { path: current.path, state }, slot);
      return false;
    });
  });
}
async function sweepAll(global: GlobalConfig): Promise<void> {
  ...
  for (const repo of repos) await sweep(global, repo, allLeaves(repo).filter(leaf => leaf.state.phase === 'merged'));
  for (const repo of repos) await sweep(global, repo, allLeaves(repo).filter(leaf => leaf.state.phase !== 'merged'));
}
async function sweep(global: GlobalConfig, repo: Repo, leaves: Leaf[]): Promise<void> {
  const ordered: Leaf[] = [...leaves].sort((a, b) => Number(b.state.phase === 'merged') - Number(a.state.phase === 'merged'));
...
export async function nextCommand(input: string | undefined): Promise<void> {
  ...
  await withLock(resolve(globalHome(), '.lock'), async () => {
    ...
    const selected: Leaf[] = input !== undefined && !existsSync(folder) ? [findLeaf(repo, input)] : leaves.filter(leaf => within(leaf.path, folder) || within(folder, leaf.path) || (leaf.state.worktree !== undefined && within(folder, leaf.state.worktree)));
    if (selected.length === 0) {
      if (input === undefined && hookPane !== undefined) return;

...
export async function install(): Promise<void> {
  ...
  const skills: string[] = readdirSync(resolve(toolRoot, 'skills'), { withFileTypes: true }).filter(entry => entry.isDirectory()).map(entry => entry.name);
  const links: Link[] = [{ source: resolve(toolRoot, 'src/akrogon.ts'), destination: resolve(homedir(), '.local/bin/akrogon') },
    ...['.claude/skills', '.agents/skills'].flatMap(root => skills.map(skill => ({ source: resolve(toolRoot, 'skills', skill), destination: resolve(homedir(), root, skill) })))];
  const conflicts: Link[] = links.filter(link => {
    const info = lstatSync(link.destination, { throwIfNoEntry: false });
    return info !== undefined && (!info.isSymbolicLink() || resolve(dirname(link.destination), readlinkSync(link.destination)) !== link.source);
  });
  ...
  for (const link of links) {
    ...
    if (lstatSync(link.destination, { throwIfNoEntry: false }) === undefined) symlinkSync(link.source, link.destination);
  }

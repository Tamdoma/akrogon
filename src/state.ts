...
import { writeYaml } from './init';

const counts = z.object({ A: z.number().int().nonnegative().default(0), B: z.number().int().nonnegative().default(0) });
export const stateSchema = z.strictObject({
  slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), phase: phaseSchema,
  created: z.string(), priority: z.string(), repo: z.string(), debate: z.enum(['yes', 'no']),
  'blocked-by': z.array(z.string()), sources: z.array(z.string()).optional(), hand_built: z.boolean().optional(),
  attempts: counts.prefault({}), done: z.array(slotSchema).default([]), fix_rounds: z.number().int().nonnegative().default(0),
  verdict: z.object({ A: verdictSchema.optional(), B: verdictSchema.optional() }).default({}),
  tab: z.string().optional(), worktree: z.string().optional(), pane: z.object({ A: z.string().optional(), B: z.string().optional() }).default({}),
...
export function leavesUnder(path: string): Leaf[] {
  ...
  return readdirSync(path, { withFileTypes: true }).filter(entry => entry.isDirectory()).flatMap(entry => leavesUnder(resolve(path, entry.name)));
}
...
export async function withRepoLock<T>(repo: Repo, action: () => Promise<T>): Promise<T> {
  // ponytail: serialize a repo's short mutations so container renames cannot split lock identities.
  // A larger scheduler can retain directory descriptors instead; this loop has at most max_active leaves.

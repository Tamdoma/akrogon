...
export const globalSchema = z.strictObject({
  ...
  harnesses: z.record(text, text), toolkits: z.record(text, text).default({}),
  repos: z.record(text, text).default({}),
}).superRefine((config, ctx) => {
  for (const slot of [config.slots.a, config.slots.b]) {
    if (!Object.hasOwn(config.harnesses, slot.harness)) ctx.addIssue({ code: 'custom', message: `Missing harness template: ${slot.harness}` });
  }
});
export const repoSchema = z.strictObject({
  remote: text.default('origin'), default_branch: text.default('main'),
  worktree_root: text.default('issues/worktrees'), rebuttal: z.boolean().default(true),
  fix_rounds: z.number().int().positive().default(3), implement: z.enum(['subagents', 'inline']).default('subagents'),
  checks: z.record(text, text).default({}), advisory: z.array(text).default([]),
  grounding: z.union([z.literal('none'), z.object({ index: text.optional(), docs: z.array(text).optional(), surfaces: z.array(text).optional(), indexed_scopes: z.array(text).optional() })]).default('none'),
...
export function globalHome(): string { return resolve(process.env.AKROGON_HOME ?? toolRoot); }
...
export function readRepo(name: string, path: string): Repo {
  ...
  return { name, root, config: repoSchema.parse(Bun.YAML.parse(readFileSync(resolve(root, 'issues/config.yaml'), 'utf8'))) };
}
...
export async function currentRepo(global: GlobalConfig, cwd: string): Promise<Repo | null> {
  ...
  for (const [name, path] of Object.entries(global.repos)) {
    ...
    if (await commonDirectory(root) === common && existsSync(resolve(root, 'issues/config.yaml'))) matches.push(readRepo(name, root));
  }
...
export function target(repo: Repo): string { return `${repo.config.remote}/${repo.config.default_branch}`; }

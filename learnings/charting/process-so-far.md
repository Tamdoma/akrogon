...
Post-handoff 2026-09-10: operator reopened two design points after handoff, `workers` switch and worker-brief upgrades. Settled by research and a slot B blind pass, applied as dated carries to the decision files and the emitted leaf files without re-charting. Lesson: a handoff is not a freeze; small additions go in as carries with the date and the evidence, and the leaves read the carry.

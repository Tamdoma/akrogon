...
| Fork      | One topic where the route splits. A fork holds one or more questions that always travel together on one screen. Taken when every material question in it is taken, and a taken fork isn't reopened.                           | one file per fork under `forks/` |

...
function bytes(path: string): string { return readFileSync(resolve(path, 'state.yaml'), 'utf8'); }
test('real same-slot and different-slot races record once and refuse stale moves unchanged', async () => {
  ...
  try {
    ...
    const same: Result[] = await Promise.all([cli(f, ['phase', 'race', 'plan.rebuttal', '--slot', 'A']), cli(f, ['phase', 'race', 'plan.rebuttal', '--slot', 'A'])]);
    ...
    const different: Result[] = await Promise.all(['A', 'B'].map(slot => cli(f, ['phase', 'other', 'plan.rebuttal', '--slot', slot])));
...
test('review aggregates verdicts, rechecks only A, caps repairs and permits operator restart', async () => {
  ...
  try {
    ...
    expect((await cli(f, ['phase', 'repair', 'check.fix', '--slot', 'B', '--verdict', 'fix'])).stdout).toBe('moved check.fix');
    ...
    expect((await cli(f, ['phase', 'repair', 'check.fix', '--slot', 'A', '--verdict', 'fix'])).stdout).toBe('moved failed');
    ...
    expect(log).toMatchObject({ from: 'check.review', to: 'check.fix', slot: 'B', verdict: { A: 'nits', B: 'fix' }, fix_rounds: 1, session: null });
    expect(Object.keys(log).sort()).toEqual(['ts', 'repo', 'slug', 'from', 'to', 'slot', 'attempts', 'fix_rounds', 'verdict', 'head', 'diff', 'session'].sort());
  } finally { f.clean(); }
});

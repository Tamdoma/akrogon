...
test('config combines defaults and repo values, reports none, and recalculates worktree base', async () => {
  ...
  try {
    yaml(resolve(f.root, 'issues/config.yaml'), { fix_rounds: 2, implement: 'inline', remote: 'upstream', default_branch: 'trunk' });
    await command(['git', 'update-ref', 'refs/remotes/upstream/trunk', 'HEAD'], f.root);
    const config = await cli(f, ['config']);
    expect(config.code).toBe(0);
    expect(Bun.YAML.parse(config.stdout)).toMatchObject({ max_active: 3, fix_rounds: 2, implement: 'inline', repo: 'repo', remote: 'upstream' });
    ...
    expect(Bun.YAML.parse((await cli(f, ['config'], worktree)).stdout)).toMatchObject({ AKROGON_BASE: first, repo: 'repo' });

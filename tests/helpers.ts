...
export function yaml(path: string, data: object): void { writeFileSync(path, Bun.YAML.stringify(data)); }
export async function fixture(): Promise<Fixture> {
  ...
  yaml(resolve(home, 'config.yaml'), { slots: { a: { harness: 'fake', model: 'strong-a', effort: 'high' }, b: { harness: 'fake', model: 'strong-b', effort: 'medium' } }, harnesses: { fake: 'fake --model {model} --effort {effort}' }, repos: { repo: root } });
  return { home, root, clean: () => rmSync(home, { recursive: true, force: true }) };
}
export async function cli(f: Fixture, args: string[], cwd: string = f.root, env: NodeJS.ProcessEnv = {}): Promise<Result> {
  const child: Bun.Subprocess<'ignore', 'pipe', 'pipe'> = Bun.spawn([process.execPath, entry, ...args], { cwd, env: { ...process.env, AKROGON_HOME: f.home, HERDR_PANE_ID: '', ...env }, stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' });
  const [stdout, stderr, code]: [string, string, number] = await Promise.all([new Response(child.stdout).text(), new Response(child.stderr).text(), child.exited]);
...
export function leaf(f: Fixture, slug: string, phase: string, extra: object = {}, container: string = 'issue'): string {
  ...
  yaml(resolve(path, 'state.yaml'), { slug, phase, created: '2026-09-10', priority: 'n', repo: 'repo', debate: 'no', 'blocked-by': [], ...extra });
  return path;
}
